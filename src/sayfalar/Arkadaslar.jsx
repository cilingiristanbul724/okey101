...
export default function Arkadaslar() {
  ...
  async function yukle() {
    ...
    const { data: kabul } = await supabase.from('arkadaslar')
      .select('*, isteyen:profiles!arkadaslar_isteyen_id_fkey(id,kullanici_adi,ad_soyad), istenen:profiles!arkadaslar_istenen_id_fkey(id,kullanici_adi,ad_soyad)')
      .eq('durum', 'Kabul')
      .or('isteyen_id.eq.' + uid + ',istenen_id.eq.' + uid)
    setArkadaslar(kabul || [])

    const { data: bekleyen } = await supabase.from('arkadaslar')
      .select('*, isteyen:profiles!arkadaslar_isteyen_id_fkey(id,kullanici_adi,ad_soyad)')
  ...
  async function istekGonder(istenenId) {
    ...
    alert(error ? error.message : 'Arkadaşlık isteği gönderildi.')
  }
    ...
    <div className="sayfa">
      ...
      {sonuclar.map(p => (
        <div key={p.id} className="kart">
          {p.kullanici_adi || p.ad_soyad}
          <button onClick={() => istekGonder(p.id)}>Arkadaş Ekle</button>
        </div>
      ))}

      {istekler.length > 0 && <h3>Gelen İstekler</h3>}
      {istekler.map(a => (
        <div key={a.id} className="kart">
          {(a.isteyen && a.isteyen.kullanici_adi) || 'Kullanıcı'} sana istek gönderdi
          <button onClick={() => kabulEt(a.id)}>Kabul Et</button>
        </div>
      ))}

      <h3>Arkadaşlarım</h3>
      {arkadaslar.length === 0 && <p>Henüz arkadaşın yok.</p>}
      {arkadaslar.map(a => {
        const k = diger(a)
        return (
          <div key={a.id} className="kart">
            {(k && (k.kullanici_adi || k.ad_soyad)) || 'Arkadaş'}
            {k && <Link to={'/ozel/' + k.id}><button>Mesaj At</button></Link>}
          </div>
        )
      })}
    </div>
